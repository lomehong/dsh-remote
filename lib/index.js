...
import { normalizeConfigInput, REMOTE_DEFAULTS } from "./config.js";
import { PairingStore } from "./tokens.js";
import { loadDevices } from "./devices.js";
import { listAddresses } from "./addresses.js";
import { startGateway } from "./gateway.js";
import { advertisedAddress, gatewayStatePath, writeGatewayState } from "./state.js";
import { verifySsoJwt } from "./sso.js";
import { resolveDeviceName } from "./hostname.js";
...
export async function apply(ctx, config) {
                ...
                try {
                    handle = await startGateway({
                        ...
                        webLaunchToken: () => {
                            ...
                            if (conn === undefined)
                                return undefined;
                            if (typeof conn.launchToken === 'string' && conn.launchToken !== '')
    ...
    ctx.inject(['webServer'], (scoped) => {
        ...
        disposers.push(web.webServer.register({ kind: 'exact', path: '/dsh-remote/api/update/apply', handler: safe(async (req, res) => {
                ...
                void updater.applyUpdate(tag, record).catch(() => { });
                sendJson(res, 200, { ok: true });
            }) }));
        // 卸载：路由全部注销 + 网关关闭。
    ...
    ctx.effect?.(() => () => clearInterval(reconcileTimer));
}
