...
- SSO 设备幂等签发（一账号一设备）：此前每次 exchange 登录都新建设备条目，
  同名「SSO <usr>」无限堆积且无法分辨当前会话。现在：
  - 同 uid 重登 → 轮换令牌（换指纹+延期），deviceId/名称稳定，旧令牌即失效
  - 旧版迁移：接管同名无 ssoUid 的存量条目（补记 uid，不新增）
  - 不同 uid 各自独立设备
- 设备记录新增 ssoUid 字段（签发账号）；过期项载入即丢弃不变
## [0.2.8] — 2026-09-08
