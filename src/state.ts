...
export function gatewayStatePath(homeDir: string): string {
...
export function advertisedAddress(
  bind: string,
  port: number,
  ifaces: AddressInfo[] = listAddresses(),
): string | undefined {
  const b = bind.trim()
  if (!isWildcard(b)) {
    if (isLoopback(b)) return undefined
    return `${b}:${port}`
  }
 ...
 */
export function writeGatewayState(homeDir: string, state: GatewayState): void {
